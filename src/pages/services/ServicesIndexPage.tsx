...
export const ServicesIndexPage: React.FC = () => {
  return (
    <PageShell title="Services Overview">
      {/* ─── HERO SECTION ─────────────────────────────────────────── */}
        ...
        <Container size="md" className="text-center">
          <FadeIn>
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-hive-yellow/40 bg-hive-yellow/10 mb-6">
              <Settings2 className="w-4 h-4 text-hive-yellow" />
        ...
        <Container size="lg">
          ...
          <div className="mt-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {allServicesData.map((service, idx) => (
              <FadeIn key={service.id} delay={idx * 0.1} className="flex">
                <div className="flex flex-col p-8 rounded-2xl bg-hive-gray border border-hive-border transition-all duration-300 hover:border-hive-yellow/50 hover:shadow-hive-hover h-full w-full">
                  ...
                  <div className="flex-1">
                    <ul className="space-y-2 mb-6">
                      {service.offerings.slice(0, 3).map((offering, i) => (
                        <li key={i} className="text-sm text-hive-text-muted flex items-start gap-2">
                          <span className="w-1.5 h-1.5 rounded-full bg-hive-yellow mt-1.5 flex-shrink-0" />
                          <span className="leading-snug">{offering.title}</span>
                        </li>
                      ))}
                  ...
                  <Button href={`/services/${service.slug}`} variant="outline" size="sm" className="w-full bg-hive-white">
                    Explore Details
